...
def generate():
    ...
    for i in range(5):
        ...
        # Keep only needed fields for API response
        d["plan"] = d["rooms"]  # we might not need plan in response, but keep for compatibility
        ...
        d["fx"] = fx
        # Build a slimmed-down response object
        concepts.append({
            "id": d["id"],
            "type": d["type"],
            "floors": d["floors"],
