...
# All-in-one file (no modules)
# 4 concepts: Concept 1, Concept 2, Concept 3, Concept 4
# =========================================================
...
def compute_cost_by_trade(d, country):
    from .forex import get_fx  # we'll inline forex below
    fx = get_fx(country)
    trades = {
...
with st.sidebar:
    ...
    if st.button("✨ Generate Concepts", use_container_width=True):
        with st.spinner("Synthesizing 4 concepts..."):   # ← changed to 4
            concepts = []
            soil_name = st.session_state.selected_soil_name
            for i in range(4):                           # ← generate 4
            ...
            log_event(username, mem, f"Generated 4 concepts. Top: {concepts[0]['id']}")
...
if nav == "Dashboard":
    ...
    c2.metric("Concepts", len(mem["designs"])*4, delta="Evolving")
